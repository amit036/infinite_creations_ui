...
function PhonePeCallbackContent() {
    ...
    const verifyPayment = async () => {
        ...
        if (!orderId) {
            setStatus('failed');
            setError('Order ID not found in callback');
            return;
        }

        try {
            // Get the merchant order ID from the order if not in URL
            let mOrderId = merchantOrderId;

            if (!mOrderId) {
                // Try to get the order details to fetch merchant order ID
                const orderDetails = await api.get(`/orders/${orderId}`);
                mOrderId = orderDetails?.paymentId;
            }

            if (!mOrderId) {
                // Payment might have been cancelled or failed before redirect
                setStatus('failed');
                setError('Payment was not completed. Please try again.');
                return;
            }

            const result = await api.post('/payments/phonepe/verify', {
                orderId,
